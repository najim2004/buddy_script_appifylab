...
export function ExploreMenu() {
  return (
    <div className="bg-card rounded-lg p-6 pb-1.5">
      <h4 className="text-title mb-6 text-base font-medium xl:text-xl">
        Explore
      </h4>
      <ul className="m-0 list-none space-y-6 p-0">
        {EXPLORE_ITEMS.map((item) => (
          <li
            key={item.name}
            className={
              item.name === "Settings" || item.name === "Save post"
                ? "mb-[15px]"
                : ""
            }
          >
            <Link
              href={item.href}
              className="text-muted-foreground hover:text-primary group flex items-center justify-between text-sm font-medium transition-colors xl:text-base"
            >
              ...
              {item.isNew && (
                <span className="border-card bg-success ml-2 rounded border px-1.5 py-0.5 text-[8px] font-semibold text-white xl:text-[10px]">
                  New
                </span>
              )}
            </Link>
