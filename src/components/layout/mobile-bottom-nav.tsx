...
const MOBILE_NAV = [
  {
    ...
    icon: (active: boolean) => (
      ...
      >
        <path
          fill={active ? "#1890FF" : "currentColor"}
          fillOpacity={active ? 1 : 0.6}
          stroke={active ? "#1890FF" : "#666666"}
          strokeWidth="1.5"
        ...
        />
        <path
          fill={active ? "#1890FF" : "#fff"}
          stroke={active ? "#1890FF" : "#fff"}
          strokeLinecap="round"
...
export function MobileBottomNav() {
  ...
  return (
    <>
      <nav
        className="border-border bg-card fixed inset-x-0 bottom-0 z-40 border-t px-5 py-4 lg:hidden"
          ...
          {MOBILE_NAV.map((item) => {
            ...
            const content = (
              <>
                {item.icon(active)}
                {item.badge > 0 ? (
                  <span className="border-card bg-primary absolute -top-1 -right-1 flex h-4 min-w-4 items-center justify-center rounded-lg border text-[10px] leading-none text-white">
                    {item.badge}
                  </span>
                ) : null}
              </>
      ...
      {menuOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="bg-background absolute inset-y-0 left-0 flex w-[min(100%,360px)] flex-col shadow-xl">
            ...
            <div className="border-border bg-card sticky bottom-0 space-y-1 border-t p-3">
              <button
                type="button"
              ...
              <button
                ...
                className="text-destructive hover:bg-muted flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-sm"
              >
...
export function MobileTopBar() {
  return (
    <header className="bg-card fixed inset-x-0 top-0 z-50 px-4 pt-4 pb-0 lg:hidden">
